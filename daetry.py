...
csv_train_x=csv.reader(open('F:/audioextract/data/dataset/trianinput.csv',encoding='utf-8'))
csv_train_y=csv.reader(open('F:/audioextract/data/dataset/trianoutput.csv',encoding='utf-8'))
...
x = tf.placeholder(tf.float32,[None,30*200])
y = tf.placeholder(tf.float32,[None,30*200])

x_audio = tf.reshape(x, [-1,30,200,1])
y_audio = tf.reshape(y, [-1,30,200,1])
#build network
...
h_pool2 = max_pool_new(h_conv2,[1, 2, 2, 1],[1, 2, 2, 1])
...
W_conv1_3 = tf.get_variable("W_conv1_3", shape=[1, 1, 20, 40], initializer = tf.truncated_normal_initializer(stddev=0.1))
b_conv1_3 = tf.get_variable("b_conv1_3",shape=[40],initializer = tf.constant_initializer(0.0001))
h_conv1_3 = tf.nn.relu(conv2d(h_pool1, W_conv1_3) + b_conv1_3)
h_pool1_3 = max_pool_new(h_conv1_3,[1, 2, 2, 1],[1, 2, 2, 1])

h_conv1_3_3 = h_pool1_3 + h_conv3
...
h_conv4 = tf.nn.relu(conv2d(h_conv1_3_3, W_conv4) + b_conv4)
...
up_sampling1 = tf.image.resize_nearest_neighbor(h_conv6, (10,40))
...
up_sampling2 = tf.image.resize_nearest_neighbor(h_conv7, (30,200))
...
with tf.Session() as sess:
    ...
    saver.restore(sess, "./model/daemodel3.ckpt")
    ...
    save_path = saver.save(sess, "./model/daemodel3.ckpt")
    print("Model saved in path: %s" % save_path)
    #plot picture
    x_audio,y_audio,prediction = sess.run([x_audio,y_audio,logits_],feed_dict={x:rows_train_x[:2],y:rows_train_y[:2]})

    fig,axes = plt.subplots(nrows = 3,ncols = 2,sharex = True,sharey = True,figsize = (20,8))
