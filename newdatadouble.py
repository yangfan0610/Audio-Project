...
def max_pool_new(x):
    return tf.nn.max_pool(x, ksize=[1, 1, 1600, 1],strides=[1, 1, 1600, 1], padding='SAME')
...
#build network
W_conv1 = weight_variable([1, 20, 1,24])  # fiter with size [1,20] for one varialbe time series
b_conv1 = bias_variable([24])
h_conv1 = tf.nn.relu(conv2d(x_audio, W_conv1) + b_conv1)

W_conv2 = weight_variable([1, 10, 24, 16])  # fiter with size [1,10] for one varialbe time series
b_conv2 = bias_variable([16])
h_conv2 = tf.nn.relu(conv2d(h_conv1, W_conv2) + b_conv2)

h_pool1 = max_pool_new(h_conv2)

flatten = tf.reshape(h_pool1, [-1,1*16*10])

W_fc1 = weight_variable([1*16*10,40])
